...
import { createGenerateDataPrompt } from '@/src/prompt/generateDataPrompt';
...
const Page = () => {
  ...
  const handleEnhance = useCallback(async () => {
    ...
    if (!areAnyApiKeysAvailable()) {
      toast.error(t('GenerateData.apiKeyError'));
      return;
    }
    if (sourceText.length > MAX_TEXT_LENGTH) {
      toast.error(t('GenerateData.maxLengthError', { maxLength: MAX_TEXT_LENGTH, currentLength: sourceText.length }));
      return;
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        toast.error(t('GenerateData.unexpectedError'));
      }
    }
    setIsLoading(false);
  }, [sourceText, t]);
        ...
        <div className='flex flex-col gap-1 w-full md:w-1/2'>
          <div className='relative w-full flex rounded-md border border-input bg-background px-1 pt-1 pb-8'>
            <TextareaAutosize
              ...
              placeholder={t('GenerateData.sourcePlaceholder')}
              onChange={(e) => {
            ...
            <div className='flex gap-2'>
              <Button onClick={handleEnhance} disabled={isLoading || !sourceText.trim()} className='w-32'>
                <>
                  ...
                  {t(enhancedText ? 'GenerateData.enhanceAgainButton' : 'GenerateData.enhanceButton')}
                </>
              </Button>
            </div>
          </div>
          ...
          <div className='w-full flex flex-col gap-3'>
            <div className='relative w-full flex rounded-md border border-input bg-gray-50 px-1 pt-1 pb-8'>
              <TextareaAutosize
                ...
                value={enhancedText}
                placeholder={
                  isLoading ? t('GenerateData.enhancedPlaceholderLoading') : t('GenerateData.enhancedPlaceholder')
                }
                disabled={true}
                forcedHeight={sharedHeight}
                onHeightChange={handleSourceHeightChange}
                ...
                {enhancedText !== '' ? (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast.success(t('GenerateData.copied'));
                    }}
                  >
                    <Copy />
                  </Button>
  ...
  return (
    <PageView
      title={{
        titleName: t('GenerateData.title'),
        description: t('GenerateData.description')
      }}
      body={renderBody()}
    />
  );
